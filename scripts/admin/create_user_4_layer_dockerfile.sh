...
# Build the Docker context and generate the User Image 4-layer Dockerfile.
...
# 4 user-image layers: lock copy, package install, source copy, run config.
...
# Runtime policy is inherited from Golden Image Base. These values record the
# selected runtime contract in the user image without counting as a user layer.
...
# 1. Dependency Lock Layer: copy only requirements.lock first for cache reuse
COPY requirements.lock ${WORKING_DIRECTORY}/requirements.lock

# 2. Python Package Layer: install pinned packages from Nexus/internal PyPI
ARG NEXUS_PYPI_URL
RUN python -m pip install --index-url "\${NEXUS_PYPI_URL}" --only-binary=:all: --requirement ${WORKING_DIRECTORY}/requirements.lock

# 3. Application Source Layer: copy user source after dependency install
COPY . ${WORKING_DIRECTORY}

# 4. Execution Config Layer: apply user-provided shell and entrypoint
USER ${RUN_AS_USER}
